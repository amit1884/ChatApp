            ...
            <>
              {item._id !== state._id ? (
                <div
                  ...
                  onClick={() => openModal(item._id, item.username)}
                >
                  <p key={item._id}>
                    <img
                      src={item.pic}
      ...
      {showModal ? (
        <div className="model-container" onClick={() => setshowModal(false)}>
          <div className="mycard text-center">
            <h5>
              Do you want to add{" "}
              <span style={{ color: "rgb(48, 3, 78)" }}>{Username}</span> as a
              friend ?
            </h5>
            <br />
            <button className="model-btn" onClick={() => AddFriend()}>
              Yes
            </button>
            &nbsp;&nbsp;&nbsp;&nbsp;
            <button className="model-btn" onClick={() => setshowModal(false)}>
              No
            </button>
          </div>
        </div>
